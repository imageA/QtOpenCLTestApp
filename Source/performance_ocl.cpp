...
{
    results << endl;
    results << setiosflags(ios_base::left);
#ifdef USE_OPENCL
    results << TAB << setw(10) << "CPU, ms" << setw(10) << "GPU, ms"
         << setw(14) << "SPEEDUP" << setw(14) << "GPUTOTAL, ms" << setw(14) << "TOTALSPEEDUP"
         << "DESCRIPTION\n";
#else
    results << TAB << setw(10) << "CPU, ms\n";
#endif
    results << resetiosflags(ios_base::left);
}
...
{
    results << setiosflags(ios_base::fixed);
...
void TestSystem::printMetrics(double cpu_time, double gpu_time, double gpu_full_time, double speedup, double fullspeedup)
{
    ...
    results << setw(14) << stream.str();
#endif
    results << cur_subtest_description_.str();
    //results << endl;
    results << resetiosflags(ios_base::left) << endl;
}
...
{
    results << TAB << "[error: " << msg << "] " << cur_subtest_description_.str() << endl;
}
...
void PerformanceOcl::set_result_test(QTextEdit* te_result)
{
    this->te_result = te_result;


    this->te_result->setText("aa");
}

void PerformanceOcl::set_tests_filter(std::string filter_name)
{
    this->filter_name = filter_name;
}

QString PerformanceOcl::start_performance_test()
{
    ...
    for (size_t i = 0; i < oclinfo.size(); i++)
    {
        ...
        {
            result << "device "<< devidx++ << "  " << oclinfo[i].DeviceName[j].c_str() << endl;
        }
    }
    ...
    TestSystem::instance().TESTINIT();
    TestSystem::instance().resetCurrentSubtest();
    TestSystem::instance().setRecordName(oclinfo[0].DeviceName[0]);

    TestSystem::instance().setTopThreshold(0);
    TestSystem::instance().setBottomThreshold(0);

    TestSystem::instance().setTestFilter(filter_name);

    TestSystem::instance().run();

    result << TestSystem::instance().results.str();

    te_result->setText(QString::fromStdString(result.str()));
